...
public class WorldGenEnderSpikes extends WorldGenerator
{
    private Block field_150520_a;
    private static final String __OBFID = "CL_00000433";

    public WorldGenEnderSpikes(Block p_i45464_1_)
    {
                ...
                for (k1 = par3 - i1; k1 <= par3 + i1; ++k1)
                {
                    for (l1 = par5 - i1; l1 <= par5 + i1; ++l1)
                    {
                        ...
                        if (i2 * i2 + j2 * j2 <= i1 * i1 + 1)
                        {
                            if(rightblock)
                            {
                                par1World.setBlock(k1, j1, l1, EndPlus.blocks.EnchEnderblock, 0, 2);
                            }
                            else
                            {
                            par1World.setBlock(k1, j1, l1, EndPlus.blocks.Enderblock, 0, 2);
                            }
                        }
                    }
                }
